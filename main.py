    ...
    def move(self, moving_left, moving_right):
        ...
        for tile in world.obstacle_list:
            ...
            if tile[1].colliderect(self.rect.x + dx, self.rect.y, self.width, self.height):
                dx = 0
                # if the ai has hit a wall then make it utrn around
            ...
            if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
                ...
                if self.vel_y < 0:  # player moving up -> jumping
                    self.vel_y = 0
    ...
    def update_animation(self):
        ...
        if self.frame_index >= len(self.animation_list[self.action]):
            if self.action == 3:  # death animation
                self.frame_index = len(self.animation_list[self.action]) - 1  # stay on the last animation frame
            else:
                self.frame_index = 0
...
class World():
    ...
    def draw(self):
        for tile in self.obstacle_list:
            tile[1][0] += screen_scroll  # this is the x-coordinate of the tile
...
class ScreenFade():
    ...
    def fade(self):
        ...
        if self.direction == 2: #vertial screen fade down
    ...
    if start_game == False:
        ...
        if player.alive:
            ...
            # throw grenade
                ...
                grenade_thrown = True
            if player.in_air:
                player.update_action(2)  # 2: in air
